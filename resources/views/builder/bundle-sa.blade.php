            ...
            <div class="mt-6">
                <form id="createForm" action="{{ route('builder.store_bundle_sa') }}" method="POST">
                    ...
                    <div class="mb-6 border-b py-4">
                        ...
                        <div class="flex flex-col my-4">
                            <label for="questions[{{ $i }}][sensitive]">Case Sensitive</label>
                            <div>
                                <label for="false_{{ $i }}">
                                    <input
                                        type="radio"
                                        name="questions[{{ $i }}][sensitive]"
                                        id="false_{{ $i }}"
                                        value="0"
                                        class="ml-2 focus:ring-0"
                                        @if(old('questions.' . $i . '.sensitive' )=='0' ) checked @else checked @endif>
                                    Not Case Sensitive
                                </label>
                                <label for="true_{{ $i }}">
                                    <input
                                        ...
                                        value="1"
                                        class="ml-2 focus:ring-0"
                                        @if(old('questions.' . $i . '.sensitive' )=='1' ) checked @endif>
                                    Case Sensitive
                                </label>
                            </div>
                            @error('questions.' . $i . '.sensitive')
                            <p class="text-sm text-red-600 font-bold mt-2">{{ $message }}</p>
                            @enderror
                        </div>
