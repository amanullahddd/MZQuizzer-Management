<x-layout>
    ...
    <div class="bg-white p-8 rounded-lg shadow-xl py-8 mt-12">
        ...
        <div>
            ...
            <p class="text-xl font-semibold">Q : {{ $filemedia->questionmedia->question }}</p>

            <div class="mt-6">
                <form action="{{ route('media.upload') }}" method="POST" enctype="multipart/form-data">
                    @csrf <!-- Token untuk keamanan CSRF -->
                    <input type="hidden" name="questionmedia_id" value="{{ $filemedia->questionmedia->id }}">
                    ...
                    <div class="flex gap-2 ml-32 mt-6">
                        <a href="{{ route('media.file') }}" class="bg-secondary bg-opacity-20 hover:bg-opacity-40 rounded-lg px-6 py-1.5 text-secondary hover:shadow-xl transition duration-150">Cancel</a>
